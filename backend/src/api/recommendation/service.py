...
from sqlalchemy import Tuple, and_, func, select
...
from backend.src.api.recommendation.schemas import GenreInfo, RecommendedFilmOut
from backend.src.models.models import Genre, MovieGenre, User, WatchHistory, Movie, MovieActor, MovieDirector
...
class RecommendationService:
    ...
    async def get_recommendations(
        ...
        personalization_weight: float = 0.34,
        genre_ids: Optional[List[int]] = None,
        exclude_genre_ids: Optional[List[int]] = None,
        year_from: Optional[int] = None,
        year_to: Optional[int] = None,
        limit: int = 20
    ) -> List[RecommendedFilmOut]:
        """
        Основной метод получения рекомендаций.
        """
        # 1. Построить базовый запрос с фильтрами
        ...
        if genre_ids:
            query = query.where(
                Movie.id.in_(
                    select(MovieGenre.movie_id).where(MovieGenre.genre_id.in_(genre_ids))
                )
            )

        # Фильтр по исключаемым жанрам
        if exclude_genre_ids:
            query = query.where(
                ~Movie.id.in_(  # NOT IN
                    select(MovieGenre.movie_id).where(MovieGenre.genre_id.in_(exclude_genre_ids))
                )
            )

        if year_from:
            query = query.filter(Movie.release_date >= date(year_from, 1, 1))
        if year_to:
            query = query.filter(Movie.release_date <= date(year_to, 12, 31))

        if popularity_weight is None:
            popularity_weight = 0.33
            # query = query.filter(Movie.kp_rating.isnot(None))

        # 2. Выполняем запрос и получаем все кандидаты
        # (на реальных данных может быть много, поэтому лучше добавить пагинацию,
        # но для простоты возьмём всех, потом отсортируем и лимитируем)
        result = await self.session.execute(query)
        movies = result.scalars().all()

        if not movies:
            return []
        movie_genres_map = await self._load_movie_genres([m.id for m in movies])

        # 3. Предварительные вычисления для нормировки
        max_votes = await self._get_max_votes()
        current_year = date.today().year
        # Минимальный год для нормировки (можно взять из данных)
        min_year = await self._get_min_year() or 1900

        # 4. Если есть пользователь, вычислим его профиль для персонализации
        user_profile = None
        if user_id:
            user_profile = await self._build_user_profile(user_id)

        # 5. Для каждого фильма вычисляем скоринги
        recommendations = []
        for movie in movies:
            pop_score = self._popularity_score(movie.vote_count, max_votes)
            new_score = self._novelty_score(movie.release_date, current_year, min_year)
            pers_score = 0.0
            if user_profile:
                pers_score = self._personalization_score(
                    movie_genres_map.get(movie.id, []),  # передаём только жанры
                    user_profile
                )

            # Взвешенная сумма
            total = (popularity_weight * pop_score +
                     novelty_weight * new_score +
                     personalization_weight * pers_score)

            # Нормализуем общий скор, если сумма весов не равна 1
            total_weight = popularity_weight + novelty_weight + personalization_weight
            if total_weight > 0:
                total /= total_weight
            ...
            recommendations.append({
                ...
                "scores": {
                    ...
                    "novelty": round(new_score, 3),
                    "personalization": round(pers_score, 3) if user_profile else None,
                    "total": round(total, 3)
                }
            })

        # 6. Сортируем по убыванию total и берём limit
        recommendations.sort(key=lambda x: x["scores"]["total"], reverse=True)
        top_recommendations = recommendations[:limit]

        # 7. Преобразуем в выходные схемы
        result = []
        for rec in top_recommendations:
            movie = rec["movie"]
            scores = rec["scores"]
            genres = [GenreInfo(id=g.id, name=g.name) for g in movie_genres_map.get(movie.id, [])]
            result.append(RecommendedFilmOut(
                id=movie.id,
                title=movie.title,
    ...
    async def _get_max_votes(self) -> int:
        """Максимальное количество голосов в базе (для нормировки)."""
        result = await self.session.execute(select(func.max(Movie.vote_count)))
        return result.scalar() or 1

    async def _get_min_year(self) -> int:
        """Минимальный год выпуска фильма в базе."""
        result = await self.session.execute(select(func.min(Movie.release_date)))
        min_date = result.scalar()
        if min_date:
            return min_date.year
        return 1900

    def _popularity_score(self, vote_count: Optional[int], max_votes: int) -> float:
        """Скор популярности на основе количества голосов (логарифмическая шкала)."""
        if not vote_count or vote_count <= 0:
            return 0.0
        # Логарифмическая нормализация, чтобы сверхпопулярные не задавливали всех
        return math.log(vote_count + 1) / math.log(max_votes + 1)

    def _novelty_score(self, release_date: Optional[date], current_year: int, min_year: int) -> float:
        """Скор новизны: чем ближе год к текущему, тем выше."""
        if not release_date:
            return 0.0
        ...
        if year <= min_year:
            return 0.0
        # Линейная интерполяция от min_year до current_year
        return (year - min_year) / (current_year - min_year)

    async def _build_user_profile(self, user_id: int) -> Dict:
        stmt = select(WatchHistory).where(
            WatchHistory.user_id == user_id,
            WatchHistory.rating.isnot(None)
        )
        ...
        movie_ids = [wh.movie_id for wh in histories]
        movie_genres_map = await self._load_movie_genres(movie_ids)

        genre_weights = {}  # без явной типизации, чтобы избежать ошибок Pylance
        for wh in histories:
            genres = movie_genres_map.get(wh.movie_id, [])
            for genre in genres:
                gid = genre.id
                if gid not in genre_weights:
                    genre_weights[gid] = (0.0, 0)
                sum_rating, count = genre_weights[gid]
                genre_weights[gid] = (sum_rating + wh.rating, count + 1)

        profile = {"genres": {}}
        for gid, (total, cnt) in genre_weights.items():
            avg = total / cnt
            profile["genres"][gid] = avg / 10.0
        return profile
    ...
    def _personalization_score(
        ...
        if not user_profile or "genres" not in user_profile:
            return 0.0
        genre_weights = user_profile["genres"]
        if not genre_weights:
            return 0.0
        movie_genre_ids = {g.id for g in movie_genres}
        if not movie_genre_ids:
            return 0.0
        total_weight = sum(genre_weights.get(gid, 0.0) for gid in movie_genre_ids)
        avg_weight = total_weight / len(movie_genre_ids)
        return min(avg_weight, 1.0)

    # ----------------- Новый метод -----------------
    async def _load_movie_genres(self, movie_ids: List[int]) -> Dict[int, List[Genre]]:
        """Загружает жанры для списка фильмов."""
        if not movie_ids:
            return {}
        stmt = (
            select(MovieGenre.movie_id, Genre)
        ...
        rows = result.all()
        movie_genres = {}
        for movie_id, genre in rows:
            movie_genres.setdefault(movie_id, []).append(genre)
        return movie_genres
